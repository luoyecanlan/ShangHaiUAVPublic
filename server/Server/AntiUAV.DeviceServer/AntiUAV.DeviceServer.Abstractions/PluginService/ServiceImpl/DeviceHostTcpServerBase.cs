...
{
    public abstract class DeviceHostTcpServerBase : IDeviceHostService
    {
        public DeviceHostTcpServerBase(ILogger<DeviceHostTcpServerBase> logger, IPeerServer peer, IMemoryCache memory)
        {
            _peer = peer;
            _memory = memory;
            _logger = logger;
        ...
        public virtual void Start()
        {
            ...
            if (dev == null || _peer == null)
                _logger.LogError("devServ tcp listion start fail.(no dev info or no serv)");
            else
            {
                _peer.UseCustomServerInfo(new PeerServerInfo($"deviceServer:{dev.Id}", dev.Lip, dev.Lport));
                _peer?.Star();
                _logger.LogInformation($"devServ tcp listion start.(devId:{dev.Id},ip:{dev.Lip},port:{dev.Lport})");
            }
        }
