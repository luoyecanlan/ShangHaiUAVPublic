...
import {request_target_info} from "../api";
...
export async function get_app_config_await() {
  return new Promise(async (res, rej) => {
    let promise = await axios.get(config_path);
    promise(_data => {
      res(_data.data);
    }).catch(err => {
      rej(err)
...
export function directNotLimit() {
  ...
  // store.dispatch('commit_login_info', null);
  // console.log('NotLimit ************ 清空');
}
...
export async function ajax_await(url='',data={},type='GET') {
  if (!Vue.prototype.$baseUrl) {
    let data = await get_app_config();
    const {api_url,infraredToken,visibleLightToken} = data;
    Vue.prototype.$baseUrl = api_url;
    Vue.prototype.$redLLight  = infraredToken;
    Vue.prototype.$visibleLight = visibleLightToken;
  }
  return new Promise(async function (resolve, reject) {
    ...
    if (type === 'GET') {
      ...
      promise = await axios.delete(url, {data: data})
    } else if (type === 'PUT') {
    ...
    try
    {
      resolve(promise.data ? promise.data : promise);
    }
    catch(error)
    {
      console.log('http錯誤',url);
      reject(error)
    }
  })
}
...
export function http_request(api,param,callback) {
  ...
  _result.then(_data =>
  {
    ...
    {
      show_message(msg_enum.error, _data.message||'Error')
      console.log('==============='+msg_enum.error, _result);
    }
  }).catch(error =>
  {
    bus.$emit(event_type.loading, false);
    console.log("========http_request _data=>api",api,'_result===>',_result,'error===>',error);
    directLogin();
  })
}

export async function http_request_await(api,param)
{
  let _result = await api(param);
  return _result.data;
}
/**
 * 请求接口统一入口方法
